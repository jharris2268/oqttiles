...
struct data_vec {
    data_vec() {}
    data_vec(xyz tile_xyz_, std::vector<std::shared_ptr<td_type>> tiles_, std::vector<xyz> to_finish_)
            : tile_xyz(tile_xyz_), tiles(tiles_), to_finish(to_finish_) {}
    xyz tile_xyz;
    std::vector<std::shared_ptr<td_type>> tiles;
    std::vector<xyz> to_finish;
};

typedef data_vec<alt_tile_data> alt_tile_data_vec;
typedef std::function<void(std::shared_ptr<alt_tile_data_vec>)> alt_tile_data_vec_cb;
std::function<void(std::shared_ptr<alt_tile_data>)> make_addblockstree_alt_cb(std::shared_ptr<geos_geometry> geom, std::vector<alt_tile_data_vec_cb> cb, int64 maxtile);
